				...
				<tbody>
					{foreach key=num item=invoice from=$invoices}
						<tr onclick="clickableSafeRedirect(event, 'viewinvoice.php?id={$invoice.id}', false)">
							...
							<td class="text-center"><span class="hidden">{$invoice.normalisedDateCreated}</span>{$invoice.datecreated}</td>
							<td class="text-center"><span class="hidden">{$invoice.normalisedDateDue}</span>{$invoice.datedue}</td>
							<td class="text-center" data-order="{$invoice.totalnum}">{$invoice.total}</td>
							<td class="text-center"><span class="label status-{$invoice.statusClass}">{$invoice.status}</span></td>
